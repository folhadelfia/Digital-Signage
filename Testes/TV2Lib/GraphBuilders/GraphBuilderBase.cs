...
namespace TV2Lib
{
	public class GraphBuilderBase : IDisposable, VideoControl.IVideoEventHandler
    {
        ...
        //For EVR
        protected TV2Lib.IMFVideoDisplayControl evrVideoDisplayControl;
        ...
        { 
            get { if (this.settings == null) this.settings = new Settings(); return this.settings; } 
			...
			get
			{
				if (audioRendererDevices == null)
				{
					...
					foreach (DsDevice d in devices)
						audioRendererDevices.Add(d.DevicePath, d);
				}
		...
		public virtual int BuildGraph()
		{
            ...
            try
            {
                ...
                //if(!useWPF)
                hr = ConfigureVMR9InWindowlessMode();
                DsError.ThrowExceptionForHR(hr);
		...
		protected virtual int AddRenderers()
		{
			return AddRenderers(this.graphBuilder);
		}
        protected virtual int AddRenderers(IFilterGraph2 graphBuilder)
        {
            int hr = 0;
            ...
            try
            {
                if (this.AudioRendererDevice != null)
                {
                    try
                    {
                        ...
                        hr = graphBuilder.AddSourceFilterForMoniker(this.AudioRendererDevice.Mon, null, this.AudioRendererDevice.Name, out tmp);
        ...
        protected virtual int ConfigureVMR9InWindowlessMode()
		{
		...
		protected virtual int ConfigureVMR9InWindowlessMode(int numberOfStream)
        {
            ...
            try
            {
                if (useEVR)
                {
                    ...
                    hr = pGetService.GetService(MFServices.MR_VIDEO_RENDER_SERVICE, typeof(TV2Lib.IMFVideoDisplayControl).GUID, out o);

                    try
                    {
                        evrVideoDisplayControl = (TV2Lib.IMFVideoDisplayControl)o;
                    }
                    catch
                    {
                ...
                else
                {
                    ...
                    if (windowlessControl != null)
                    {
                        ...
                        hr = windowlessControl.SetVideoClippingWindow((this.hostingControl as VideoControl).Handle);
        ...
        #region parede de código comentado
